...
import { 
  ...
  collectFactors, 
  processStockData
} from '../utils/stockFilterUtils.js'
import { DEFAULT_CODE_COLUMNS } from '../utils/stockFormatters.js'
...
export function useStockFilter() {
  ...
  const handleSearch = async (filters = { ...currentFilters }, options = {}) => {
    ...
    try {
      ...
      console.log('搜索完成，数据状态:', {
        stockListLength: stockList.value.length,
      ...
      searchLoading.value = false
      console.log('搜索状态已重置:', { searchLoading: searchLoading.value })
    }
  }
  ...
  const handleStockCodeClick = (stockCode) => {
    console.log('股票代码点击:', stockCode)
    // 可以在这里添加额外的逻辑，比如统计点击次数等
  }

...
export async function getThemesData(getAllThemesApi) {
  ...
  if (cachedThemes && !themesCache.shouldUpdate()) {
    console.log('使用缓存的题材数据:', cachedThemes.length, '个题材')
    return {
      themes: cachedThemes,
      fromCache: true,
  ...
  themesCache.loadPromise = (async () => {
    try {
      ...
      console.log('题材数据更新完成:', themes.length, '个题材')
      return {
        themes,
        fromCache: false,
        lastUpdate: themesCache.lastUpdateTime
...
export async function preloadThemesData(getAllThemesApi) {
  try {
    ...
    if (cacheInfo.shouldUpdate) {
      console.log('预加载题材数据...', cacheInfo)
      // 立即加载，确保界面刷新时有数据可用
      const result = await getThemesData(getAllThemesApi)
      
      // 如果检测到页面刷新，记录日志
      if (themesCache.isPageRefresh()) {
        console.log('检测到页面刷新，已更新题材数据')
      }
      
      return result
    } else {
      console.log('题材数据已是最新，无需预加载', {

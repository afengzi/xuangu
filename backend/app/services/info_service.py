    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                error_msg = error_data.get('detail', f'外部API返回错误状态码: {response.status_code}')
                print(f"外部API错误信息: {error_msg}")
                return {'error': True, 'message': error_msg}
            except:
                return {'error': True, 'message': f'外部API返回错误状态码: {response.status_code}'}
    except Exception as e:
        print(f"获取股票代码 {code} 信息失败: {e}")
        return {'error': True, 'message': f'获取股票信息失败: {str(e)}'}